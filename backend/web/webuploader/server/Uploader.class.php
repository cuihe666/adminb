...
//        $this->stateMap['ERROR_TYPE_NOT_ALLOWED'] = iconv('unicode', 'utf-8', $this->stateMap['ERROR_TYPE_NOT_ALLOWED']);
    ...
    private function upFile()
    {
        ...
        //判断---如果当前调用上传图片控件的是专题活动分享图片的话，判断图片格式必须为1:1的格式（宽/高=1）
        if($this->params['upload_type']."_".$this->params['upload_id']=="thematic_show"){
            if($img_size[0]/$img_size[1]!=1){
                return $this->stateInfo=$this->stateMap["ERROR_IMG_SIZE"];
            }
        }
        // 上传到七牛后保存的文件名（命名规则：前缀-id-time().6位随机数）付燕飞2017年3月23日11:01:59修改
        $key = $this->params['upload_type']."_".$this->params['upload_id']."_".time().rand(10000,99999).$this->fileType;
