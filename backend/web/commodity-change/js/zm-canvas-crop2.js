...
ZmCanvasCrop.prototype = {
	...
	readFile: function(){
		...
		function readFile(){
			...
			if($(".total_div").length > 6){
				alert("最多上传5张图片")
				return false
			}else{
				var len = $(".after_div").length;
				var len2 = $(".zp ul li").length;
				var num = len2;
				if(num > 5){
					alert("最多上传5张图片");
					return false
			...
			if(!/image\/\w+/.test(file.type)){
				...
				var f=document.getElementById("ipt").value;
	...
	drawCavDown: function(src){
		...
		function drawDown(){
			...
			$(".canvas-box").width(self._img_show.width)
			$(".canvas-box").height(self._img_show.height)

			$(".layer_con_cont").width(self._img_show.width)
			$(".layer_con_cont").height(self._img_show.height)
			//裁剪区域透明
			$ctx.beginPath();
	...
	upCanvasEvent: function(){
		...
		function cavMouseDown(e){
			...
			$(document).on('mousemove', function(e){
				...
				$(canv).css({left: countPosition().left, top: countPosition().top });//移动上层canvas偏移的距离


				var L = $(".overlay").position().left;
				var T = $(".overlay").position().top;
				var num = $(".overlay").width()
				var num2 = $(".overlay").height()
				//console.log(L,'left',num,"width",(num-90)/2+L,$("#icon_save").offset().left);
				//自己注释的
				$("#icon_save").css({"top":num2+T+20,"left":(num-90)/2+L})//设置保存按钮的位置
	...
	addResizeBtn: function(){
		...
		var L = $(".overlay").position().left;
		var T = $(".overlay").position().top;
		$("#icon_save").css({"top":self._img_show.crop_height+20,"left":(self._img_show.crop_width-90)/2+L})//设置保存按钮的位置
	...
	save: function(){
		...
		var str = "<div class='tool_div'><p>"+
			"<span class='del'>删除</span>"+
			"<span class='shoutu shoutu2'>设为</br>首图</span>"+
			"</p>"+
			"<input type='hidden' name=title_pic[] value='"+base64Url+"'>"+
			"</div>"+
			"<div class='div_shoutu'>"+
			"<i></i>"+
			"<b>首图</b>"+
			"</div>"

		var div = $("<li class='total_div tg_st'></li>")
		div.append($result)
		div.append(str)
		$(".add").before(div)
		var len = $(".zp ul .total_div").length;
		if(len == 1){
			$(".zp ul").find(".div_shoutu").addClass("current_first")
			$(".zp ul .tool_div").find(".shoutu").hide()
		}
	...
	resizeCrop: function(real){//剪裁框大小
		...
		//console.log(this._img_show.crop_width)
	}
