...
$(function(){
                    ...
                    if(res.pageInfo.list.length == 0){
                        ...
                        $("#export_all").on("click", function(){
                            ...
                            $("#myModa33 .confirm").on("click", function() {
                                $("#myModa33").modal("hide");
                                var all_url = http_url + export_init + "?orderType=";
                                ...
                                $(this).on("click", function(){
                                    ...
                                    $(".md12_footer").html("")
                                        ...
                                        add_btn = '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal3">确认退款</button>'+
                                            '<button type="button" class="btn btn-primary" onclick="reject()" data-toggle="modal" data-target="#myModal4">拒绝退款</button>'
                                        $(".md12_footer").append(add_btn);
                                    }else{
                                    ...
                                    if($(this).html() == "退款详情" ){
                                        ...
                                        // console.log(http_url + pay_url + pay_id)
                                        $.ajax({
                                            type: "GET",
                                            contentType:"application/json;charset=UTF-8",
                                            ...
                                            success: function(res){
                                                if(res.code == 0){
                                                    ...
                                                    $(".js_pay td").eq(33).html(overManFee); // 超员费
    ...
    // 点击确认退款按钮  .dtk_y
    $(".dtk_y").on("click", function(){
        var url = "/api/order/house/audit/agree";
        var data = {
            orderId:$(".m_id").val(),
            optId:adminId
        }
    ...
    // 点击拒绝按钮  .dtk_n
    $(".dtk_n").on("click", function(){
        var url = "/api/order/house/audit/refuse";
        var data = {
            orderId:$(".m_id").val(),
            optId:adminId,
            refuseReason:$("#m_textarea").val()
        }
