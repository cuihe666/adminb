$(function(){
	...
	if(_id == 6){
		...
		setInterval(function(){
			for(var i=1;i<$("#jump_type option").length;i++){
				if($("#jump_type option").eq(i).val() != "12"){
	...
	if(_id == 5){
		...
		setInterval(function(){
			for(var i=1;i<$("#jump_type option").length;i++){
				if($("#jump_type option").eq(i).val() != "18"){
					$("#jump_type option").eq(i).remove();
				}
			}
		},200);
	}
	//console.log(_id); return false;
	// 请求数据
	...
	$("#savebut").on("click", function(){
		...
		if(linkType == 0){
			layer.alert("请选择跳转类型!");
			return false;
		}else if(linkType == 1 || linkType == 12 || linkType == 15 || linkType == 18 || linkType == 14){
			...
			for(var i = 0; i < $(".shi option").length; i++){
				if($(".shi option").eq(i).val() == city){
		...
		if($(this).parent().find("input").eq(0).val() == 0){
			...
			$.ajax({
				type: "POST",
				contentType:"application/json;charset=UTF-8",
				url: http_url + "/api/operate/addOperateRecommend",
				data: JSON.stringify(dat),
				success: function(res){
					if(res.code == "0"){
			...
			// 编辑
			dat.id = $(this).parent().find("input").eq(1).val();
			$.ajax({
				type: "POST",
				contentType:"application/json;charset=UTF-8",
				url: http_url + "/api/operate/updateByProductId",
				data: JSON.stringify(dat),
				success: function(res){
					if(res.code == "0"){
						// window.location.href = "index.html";
	...
	function ajax_post(url, dat){
		$.ajax({
			...
			success: function(res){
				...
				try{
					 $("table tbody").remove();
					var arr_sort = ["1", "12", "15", "18", "14", "2", "17", "16"];
					var arr_cont = ["H5", "民宿产品详情", "酒店产品详情", "旅游产品详情","原生酒店", "民宿列表(城市分)", "酒店列表(城市分)", "旅游列表(城市分)"];
					for(var i = 0; i < res.data.length; i++){
						...
						var str = "<tr>" +
								  "<td>" + (i + 1) + "</td>" + 
								  //"<td>" + arr_cont[arr_sort.indexOf(res.data[i].linkType)] + "</td>" +
