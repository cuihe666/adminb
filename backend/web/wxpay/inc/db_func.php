...
function get_ticket_order($out_trade_no, $total_fee)
{
    ...
    global $empire;
    $list = $empire->select("select * from `order_ticket` WHERE state=1 AND order_no='" . $out_trade_no . "' and total_price='" . $total_fee . "' limit 1");
    if (empty($list)) {
        return false;
    }
    return $list;
...
function upOrder($out_trade_no, $total_fee, $trade_status, $house_id, $in_time, $out_time, $trade_no, $order_uid, $house_uid, $plat)
{
    ...
    foreach ($arrTime as $k => $v) {
        ...
        $detault_stock = $empire->select("select * from house WHERE house_id=$house_id limit 1");
        if ($old_data[0]) {
            $s_stock=$old_data[0]['stock'] - 1;
            $str=$s_stock.'---'.$detault_stock[0]['totalStock']."\r\n";
            if (intval($s_stock) == intval($detault_stock[0]['totalStock'])) {
    ...
    if ($in_time == date("Y.m.d", time()) && time() > $ruzhu_time) {
        $sql2 = "update `order` set status = 2, update_time = '" . time() . "', pay_time = '" . time() . "', Payment = 1 where order_num = '" . $out_trade_no . "'";
    } else {
        $sql2 = "update `order` set status = 1, update_time = '" . time() . "', pay_time = '" . time() . "', Payment = 1 where order_num = '" . $out_trade_no . "'";
    }
    //更新house
...
VALUES ('" . $trade_no . "', '" . $out_trade_no . "', $plat,'" . $trade_status . "','" . $total_fee . "',$order_uid,$house_uid,0," . time() . ")";
    ...
    $sql5 = "INSERT INTO `account_detail` (user_id,money,ctime,type,event_id) VALUES ($order_uid,$total_fee,'" . time() . "',3,$order_id)";
    $res = mysql_query($sql1);
    if(!$res){
        file_put_contents('./error.txt','1'.mysql_error()."\r\n",FILE_APPEND);
    }
    $res1 = mysql_query($sql2);
    if(!$res1){
        file_put_contents('./error.txt','2'.mysql_error()."\r\n",FILE_APPEND);
    }
    $res2 = mysql_query($sql3);
    if(!$res2){
        file_put_contents('./error.txt','3'.mysql_error()."\r\n",FILE_APPEND);
    }
    $res3 = mysql_query($sql4);
    if(!$res3){
        file_put_contents('./error.txt','4'.mysql_error()."\r\n",FILE_APPEND);
    }
    $res4 = mysql_query($sql5);
    if(!$res4){
        file_put_contents('./error.txt','5'.mysql_error()."\r\n",FILE_APPEND);
    }
...
function up_travel_order($out_trade_no, $total_fee, $trade_status, $trade_no, $msg)
{
    ...
    $sql2 = "insert into `order_flow_pay` (order_no,trade_no,trade_status,return_msg,total_fee,create_date,pay_type) VALUES ('{$out_trade_no}','{$trade_no}','{$trade_status}','{$msg}','{$total_fee}','{$date}',1)";
...
function refunds($out_trade_no, $total_fee, $trade_status, $house_id, $in_time, $out_time, $trade_no, $order_uid, $house_uid, $plat)
{
    ...
    global $empire;
    mysql_query("BEGIN");
    $sql1 = "update `order` set status = 4 ,Payment=1, update_time = '" . time() . "' where order_num = '" . $out_trade_no . "'";
...
function notRefunds($out_trade_no, $trade_no, $total_fee, $trade_status, $plat)
{
    global $empire;
    $sql1 = "update `order` set status = 4 ,Payment=1, update_time = '" . time() . "' where order_num = '" . $out_trade_no . "'";
    $sql2 = "INSERT INTO `pay` (order_id, order_num, plat,play_status,money,type,ctime,is_ok)
VALUES ('" . $trade_no . "', '" . $out_trade_no . "', $plat,'" . $trade_status . "','" . $total_fee . "',1," . time() . ",2)";
    mysql_query($sql1);
    mysql_query($sql2);
    return true;
}
...
function send_sms1($mobile, $content)
{
    $username = 'N5879106';
    $pwd = 'UnLQ0ykXSua0b0';
