...
<div class="booking-index">
                    ...
                    <div class="ibox-content" style="margin-bottom: 20px;">
                        <div class="search-box clearfix">
                            <div class="search-item">
                                <?= Html::beginForm(['travel-note-manage/index'], 'get', ['enctype' => 'multipart/form-data', 'class' => 'form-horizontal', 'id' => 'addForm']) ?>
                                <div class="search-item">
                                ...
                                <div class="search-item">
                                    <div class="input-group" style="width: 120px; float: left;margin-right: 10px;">
                                        ...
                                        <?= Html::a("清空",$url = ['travel-note-manage/index'],$options = ['class' => 'btn btn-sm btn-primary']) ?>
                                    </div>
                                </div>
                                <?= Html::endForm() ?>
                            </div>
                        ...
                        $gridColumns = [
                            ...
                            ['attribute' => 'account',
                                ...
                                'value' => function ($model) {
                                    //2017年6月23日16:06:20 付燕飞加判断。如果$model->uid不为空的时候再调用下面获取账号名称的方法。
                                    if($model->uid) {
                ...
                <div class="moda1_sort">
                    <div class="modal-body" style="overflow: hidden;">
                        ...
                        <p>
                            ...
                            <span class="note_name"></span>
                        </p>
                        <p>
                            <label style="float: left">排序值：</label>
                            <input type="number" class="modal_text app_link note_sort" name="note_sort" value="0" oninput="if(value.length>1){value=10}" min="0" max="10" >
                    ...
                    <div class="modal-footer">
                        <input type="hidden" name="note_id" class="m_note_id" value="" />
    ...
    <script>
        ...
        $(".f_sort").click(function(){
            ...
            $.ajax({
                type: 'post',
                url: "<?=Url::to(['travel-note-manage/get-note-info']) ?>",
                data: {
                    "note_id":id,
                },
                success: function (data) {
                    console.log(data);
                    data = eval("("+data+")");
                    $(".moda1_sort .note_name").text(data.name)
                    $(".moda1_sort .note_sort").val(data.sort);
        ...
        $(".m_commit").click(function(){
            ...
            $.post("<?=Url::to(["travel-note-manage/update-sort"])?>", {
        ...
        $(".f_reject").click(function(){
            ...
            $(".moda1_reject").show();
            $("#more_modal").modal("show");
            $(".note_name_r").text($(this).parents("tr").find("td").eq(6).text());
            $(".moda1_reject .m_note_id").val(id);
        })

        //排序弹框提交按钮ajax处理数据
        $(".m_commit_r").click(function(){
            var m_reason = $(".m_reason").val();
            var note_id = $(".moda1_reject .m_note_id").val();
            if(m_reason==""){
                $(".m_reason_e").text("请填写驳回原因");
                return false;
            }
            $.post("<?=Url::to(["travel-note-manage/update-status"])?>", {
                "PHPSESSID": "<?php echo session_id();?>",
                "<?=Yii::$app->request->csrfParam?>": "<?=Yii::$app->request->getCsrfToken()?>",
                data: {
                    "note_id":note_id,
