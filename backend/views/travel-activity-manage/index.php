...
<div class="booking-index">
                    ...
                    <div class="ibox-content" style="margin-bottom: 20px;">
                        <div class="search-box clearfix">
                            <div class="search-item">
                                <?= Html::beginForm(['travel-activity-manage/index'], 'get', ['enctype' => 'multipart/form-data', 'class' => 'form-horizontal', 'id' => 'addForm']) ?>
                                <div class="search-item">
                                ...
                                <div class="search-item">
                                    <div class="input-group" style="width: 115px; float: left;margin-right: 10px;">
                                ...
                                <div class="search-item">
                                    <div class="input-group" style="width: 120px; float: left;margin-right: 10px;">
                                        <?= Html::activeDropDownList($searchModel, 'type', Yii::$app->params['travel_activity_type'], ['class' => 'form-control', 'prompt' => '按活动类型']) ?>
                                    </div>
                                </div>
                                <div class="search-item">
                                    <div class="input-group" style="width: 110px; float: left;margin-right: 10px;">
                                ...
                                <div class="search-item" style="overflow: hidden">
                                    <div class="input-group" style="width: 110px; float: left;margin-right: 10px;">
                                        ...
                                        <?= Html::a("清空",$url = ['travel-activity-manage/index'],$options = ['class' => 'btn btn-sm btn-primary']) ?>
                                    </div>
                                </div>
                                <?= Html::endForm() ?>
                            </div>
                        ...
                        $gridColumns = [
                            ...
                            ['attribute' => 'type',
                                'header' => '活动类型',
                                'value' => function ($model) {
                                    return Yii::$app->params['travel_activity_type'][$model->type];
                                }
                            ],
                                ...
                                'buttons' => [
                                    ...
                                    'online' => function ($url, $model, $key) {
                                        ...
                                        if($time>=$start && $time<=$end){
                                            if($model->status==1){
                                                return Html::a("下线", "javascript:;", ['class' => 'btn btn-danger  m_online', 'id' => $key,'data' => $model->status,'start'=>$start,'end'=>$end,'style'=>'margin-right:5px;']);
                                            }
                                            if($model->status==2){
                                                return Html::a("上线", "javascript:;", ['class' => 'btn btn-success m_online', 'id' => $key,'data' => $model->status,'start'=>$start,'end'=>$end,'style'=>'margin-right:5px;']);
                                            }
                                        }
    ...
    <style>
        ...
        .activity_name{ width:500px; height: 30px; line-height: 30px; display: inline-block;}
            ...
            <div class="modal-content">
                ...
                <div class="moda1_sort">
                    <div class="modal-body" style="overflow: hidden;">
                        ...
                        <p>
                            ...
                            <span class="activity_name"></span>
                        </p>
                        <p>
                            <label style="float: left">排序值：</label>
                            <input type="number" class="modal_text app_link activity_sort" name="activity_sort" value="0" oninput="if(value.length>1){value=10}" min="0" max="10" >
                    ...
                    <div class="modal-footer">
                        <input type="hidden" name="activity_id" class="m_activity_id" value="" />
                ...
                <div class="moda1_reject">
                    <div class="modal-body" style="overflow: hidden;">
                        ...
                        <p>
                            <label style="float: left">标题名称：</label>
                            <span class="activity_name_r"></span>
                        </p>
                        <p>
    ...
    <script>
        ...
        $(".f_sort").click(function(){
            ...
            $.ajax({
                type: 'post',
                url: "<?=Url::to(['travel-activity-manage/get-activity-info']) ?>",
                data: {
                    "activity_id":id,
                },
                success: function (data) {
                    data = eval("("+data+")");
                    $(".moda1_sort .activity_name").text(data.name)
                    $(".moda1_sort .activity_sort").val(data.sort);
                    ...
                    $(".moda1_sort .m_activity_id").val(id);
        ...
        $(".m_commit").click(function(){
            ...
            var activity_id = $(".moda1_sort .m_activity_id").val();
            ...
            $.post("<?=Url::to(["travel-activity-manage/update-sort"])?>", {
        ...
        $(".f_reject").click(function(){
            ...
            $(".moda1_reject").show();
            $("#more_modal").modal("show");
            $(".activity_name_r").text($(this).parents("tr").find("td").eq(6).text());
            $(".moda1_reject .m_activity_id").val(id);
        })

        //排序弹框提交按钮ajax处理数据
        $(".m_commit_r").click(function(){
            var m_reason = $(".m_reason").val();
            var activity_id = $(".moda1_reject .m_activity_id").val();
            if(m_reason==""){
                $(".m_reason_e").text("请填写驳回原因");
                return false;
            }
            $.post("<?=Url::to(["travel-activity-manage/update-status"])?>", {
                "PHPSESSID": "<?php echo session_id();?>",
                "<?=Yii::$app->request->csrfParam?>": "<?=Yii::$app->request->getCsrfToken()?>",
                data: {
                    "activity_id":activity_id,
        ...
        $(".m_online").click(function(){
            ...
            if(now_time >= start && now_time <=end) {
                ...
                layer.alert('不在开始和结束时间内，无法操作', {icon: 2, title: '友情提示'});
            }
        })
