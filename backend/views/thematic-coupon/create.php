...
<div class="user-backend-create">
    <div class="user-backend-form">
        ...
        <ul class="thematic_ul">
            <li>
                <label>优惠券礼包名称：</label>
                <input type="text" class="input_text" maxlength="15" name="Coupon1Activity[title]" value="" />
                <i class="error_name prom">最多不超过15字</i>
            </li>
            <li>
                <label>预设库存量：</label>
                <input type="number" class="input_text storage" max="10000000" min="1" oninput="if(value>10000000) value=10000000" name="Coupon1Activity[storage]" value="1" />
                <i class="error_storage prom">最大可设置10000000</i>
            </li>
            <li>
                <label>每日领取的最大数量：</label>
                <input type="number" class="input_text" max="100000" min="0" oninput="if(parseInt(value)>parseInt($('.storage').val())){value=$('.storage').val();}" name="Coupon1Activity[daily_max]" value="0" />
                <i class="error_creator prom">最大不可超过预设库存量的值【0为不限制】</i>
            </li>
            <li>
                <label>发放开始时间：</label>
                <input id="d422" name="Coupon1Activity[start_time]" value="" class="input_text" type="text" onfocus="var d4312=$dp.$('d4312');WdatePicker({readOnly:true,minDate:'%y-%M-{%d} 00:00:00',maxDate:'#F{$dp.$D(\'d4312\')}',dateFmt:'yyyy-MM-dd 00:00:00',onpicked:function(){d4312.focus()}})" placeholder="请设置开始时间" readonly="">
                <i class="error_start_time"></i>
            </li>
            <li>
                <label>发放结束时间：</label>
                <input id="d4312" class="input_text" name="Coupon1Activity[end_time]" value="" type="text" placeholder="请设置结束时间" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'d422\')}',dateFmt:'yyyy-MM-dd 00:00:00',readOnly:true})" readonly="">
            ...
            <li>
                <label>发放对象：</label>
                <select name="Coupon1Activity[object]" class="coupon_select">
            ...
            <div class="thematic_submit">
                ...
                <?= Html::submitButton('添加', ['class' => 'btn btn-success sub']) ?>
            </div>
        </ul>
        <?= Html::endForm();?>
...
<script>
    $(".thematic_submit").on("click",".sub",function(){
        ...
        if(daily_max.length==0){
            ...
            $(".error_daily_max").text("最大不可超过预设库存量的值【0为不限制】").css("color","#aaa");
        }

        if(coupon_activity_id.length==0){
            $(".error_coupon_batch").text("请添加优惠券批次").css("color","red");
            return false;
        }
        else{
            $(".error_coupon_batch").text("");
        }
...
<style>
    ...
    .coupon_list p.coupon_info span{ margin-right: 3px;}
