...
<div class="booking-index">
    <ul class="title">
        ...
        <li><a href="<?=\yii\helpers\Url::to(['travel-tag-manage/activity'])?>">当地活动</a></li>
        ...
        <li class="on"><a href="<?=\yii\helpers\Url::to(['travel-tag-manage/note'])?>">游记</a></li>
    </ul>
    <div class="tap-wrap">
            ...
            ['class' => 'yii\grid\ActionColumn',
                ...
                'buttons' => [
                    'update' => function ($url, $model, $key) {
                        return '<a href="javascript:;" id="'.$key.'" data-status="'.$model->status.'"  p_id="'.$model->travel_tag_id.'" class="btn btn-danger update">修改</a>';
                    },
                    'online' => function($url, $model, $key){
                        if($model->status == 1){
                            return '<a href="javascript:;" id="'.$key.'" p_id="'.$model->travel_tag_id.'" class="btn btn-danger line">禁用</a>';
                        }else if($model->status == 0){
                            return '<a href="javascript:;" id="'.$key.'"  p_id="'.$model->travel_tag_id.'" class="btn btn-danger online">启用</a>';
                        }
                    }
                ],
            ],
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" style="overflow: hidden;">
                <h4 class="modal-title t_name" id="myLargeModalLabel"></h4>
                <p>
                    <label style="float: left">类目：</label>
                    <select name="" id="selectedtag">
                        <?php
                            foreach($options as $k=>$v){
                                ?>
                                <option class="tagid" value="<?=$v['id']?>"><?=$v['title']?></option>
                            <?php
                            }
                        ?>
                    </select>
...
<script>
    $(function(){
        ...
        $('#sub').click(function(){
//            var tag = $('.tagid').find('option:selected').val();
...
//            console.log(tag);return false;
            ...
            if(title.length != 0){
                $.ajax({
                    type: 'post',
                    url: "<?php echo \yii\helpers\Url::toRoute(['travel-tag-manage/note-store']) ?>",
                    data: {tag:tag,title:title,desc:desc,sort:sort,status:status,store_type:store_type,id:id},
...
//                            window.reload();
        ...
        $('.line').click(function(){//禁用
            ...
            $.ajax({
                type: 'post',
                url: "<?php echo \yii\helpers\Url::toRoute(['travel-tag-manage/note-getchange']) ?>",
...
//                    console.log(data);return false;
                    ...
                    if(data.status==-1){
                        ...
                        if(return_data.length!=0){
                            layer.confirm("已有"+data.nums+"篇游记选择此标签，是否将已选标签替换：<select name='change_item' class='change_item'>"+return_data+"</select><p>禁用后此标签在商家后台和APP端将不再显示，确定禁用吗？</p>", {icon: 3, title: '友情提示'}, function (index) {
                                var items = $('.change_item').find('option:selected').val();
                                $.ajax({
                                    type: 'post',
                                    url: "<?php echo \yii\helpers\Url::toRoute(['travel-tag-manage/note-line']) ?>",
                                    data: {id:id,type:type,change_item:items,model:data.field},
                                    success: function (data) {
//                                        console.log(data);return false;
                                        if(data==-1){
                                            layer.alert('参数有误', {icon: 2});
        ...
        $('.update').click(function(){//修改
            var tag = $(this).attr('p_id');
            var title = $(this).parent().parent().siblings(":eq(1)").html();
            var desc = $(this).parent().parent().siblings(":eq(2)").html();
            var sort = $(this).parent().parent().siblings(":eq(3)").html();
            var status = $(this).attr('data-status');
            var id = $(this).parent().parent().parent().attr('data-key');
//            console.log($('#selectedtag').find("option[value='"+tag+"']"));    return false;
            $('#selectedtag').find("option[value='"+tag+"']").attr("selected",true);
