                    ...
                    <div class="ibox-content" style="margin-bottom: 20px;">
                        <div class="search-box clearfix">
                            <div class="search-item">
                                <?= Html::beginForm(['travel-note/index'], 'get', ['enctype' => 'multipart/form-data', 'class' => 'form-horizontal', 'id' => 'addForm']) ?>
                                <div class="search-item">
                                ...
                                <div class="search-item">
                                    <div class="input-group" style="width: 120px; float: left;margin-right: 18px;">
                                        ...
                                        <?= Html::a("清空",$url = ['travel-note/index'],$options = ['class' => 'btn btn-sm btn-primary']) ?>
                                    </div>
                                </div>
                                <?= Html::endForm() ?>
                            </div>
                            ...
                            ['class' => 'yii\grid\ActionColumn',
                                ...
                                'buttons' => [
                                    'view' => function ($url, $model, $key) {
                                        if ($model->status == 0) {
                                            return Html::a('审核', ['travel-note/view', 'id' => $key], ['class' => 'btn btn-info']);
                                        }
                                        if ($model->status == 3) {
                                            return Html::a('查看', ['travel-note/view', 'id' => $key], ['class' => 'btn btn-info']);
                                        }
                                        if ($model->status == 1) {
                                            return Html::a('下线', "javascript:drop($key);", ['class' => 'btn btn-danger', 'id' => $key]) . Html::a('查看', ['travel-note/view', 'id' => $key], ['class' => 'btn btn-info']);
                                        }
                                        if ($model->status == 2) {
                                            return Html::a('上线', "javascript:online($key);", ['class' => 'btn btn-danger', 'id' => $key]) . Html::a('查看', ['travel-note/view', 'id' => $key], ['class' => 'btn btn-info']);
                                        }
                                    },
                                ],
                            ],
    ...
    <script>
        ...
        function online($key) {
            layer.confirm('确认要上线吗', {icon: 3, title: '友情提示'}, function (index) {
                $.post("<?=Url::to(["travel-note/online"])?>", {
                    "PHPSESSID": "<?php echo session_id();?>",
                    "<?=Yii::$app->request->csrfParam?>": "<?=Yii::$app->request->getCsrfToken()?>",
                    data: {id: $key},
                }, function (data) {
