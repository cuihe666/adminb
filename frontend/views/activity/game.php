...
<div class="layer" id="layer">
    <div class="layer_con">
        ...
        <div class="reaward">
            <img src="" alt="">
            <div>恭喜你抽中优惠券<span id="award" style="display: none;"></span>,请记得下单时使用哦！</div>
        </div>
...
<div class="layer2" id="layer2">
    <div class="layer_con">
        ...
        <div class="reaward">
            <img src="" alt="">
            <div>恭喜你抽中<span id="award2"></span>,客服将在10个工作日内联系您领取奖品哦！</div>
        </div>
        <img src="<?= Yii::$app->request->baseUrl ?>/images/close.png" alt="" class="close">
    </div>
</div>
        ...
        <ul class="award_li">
            <?php if (!empty($award_list)): ?>
                <?php foreach ($award_list as $k => $v): ?>
                    <li><?php echo $v['create_time'] ?> 恭喜您抽中<?php echo $v['title'] ?>！</li>
                <?php endforeach; ?>
            <?php endif ?>
        </ul>
    </div>
...
<div class="main">
    <div class="top">
        ...
        <span style="font-size:18px;">幸运大转盘</span>
...
<script src="<?= Yii::$app->request->baseUrl ?>/js/kinerLottery1.js"></script>
...
<input type="hidden" value="" class="deg">
...
//                javascript:jsandroid.alipay();
    ...
    $(function () {
        ...
        $('.share').click(function () {
            ...
            var title = '棠果旅居节';
            var content = '动动手指，即刻领取免费欧洲游，还有iPhone7、现金红包等你拿哟';
...
//  deg = -deg;
//        console.log(deg, 1111)

//      if ((deg > 330 && deg <= 360)) {
        if ((deg > 330 && deg <= 360) || (deg > 0 && deg <= 30)) {

            //        var deg = Number(deg) + 330
...
//				var  this_arr = [60,180,240];
//              var this_deg = this_arr[i][Math.floor(Math.random() * 2)];
//              return this_deg;
//          }
// 			function random() {
//              var  this_arr = [[30,90],[150,210],[210,270]];
//              var  this_arr_sub = this_arr[Math.floor(Math.random()*2)];
//              var this_deg = Math.floor(Math.random()*this_arr_sub[1]+this_arr_sub[0]);
//              return this_deg;
//          }
            var jg;
            var uid = $('.uid').val();
...
//
                            ...
                            $.ajax({
                                type: 'post',
                                url: "<?php echo \yii\helpers\Url::to(['getdeg']) ?>",
                                data: {"_csrf-frontend": "<?php echo Yii::$app->request->csrfToken ?>"},
                                async: false,
                                success: function (data) {
//                                    var this_arr_sub = $.parseJSON(data);
//                                    var this_deg = Math.floor(Math.random() * (this_arr_sub[1] - this_arr_sub[0]) + this_arr_sub[0]);
//                                    document.title = data;
                                    jg = data;
//                                    $('.deg').val(jg);
//                                    _this.goKinerLottery(jg);
                                }
                            })
                        }
                    }
            ...
            if ((deg > 330 && deg <= 360) || (deg > 0 && deg <= 30)) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var time = FormatDate();
                        $('.award_li').append("<li>" + time + " 恭喜您抽中棠果杯子！</li>");
                        $(".reaward img").attr("src", "<?= Yii::$app->request->baseUrl ?>/images/tg_cup.png").css({
                            "width": "50%",
                            "margin-left": "25%"
                        })
                    }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        document.getElementById("award").innerHTML = whichAward(deg);
                        var time = FormatDate();
                        $('.award_li').append("<li>" + time + " 恭喜您抽中10元现金券！</li>");
                        $(".reaward img").attr("src", "<?= Yii::$app->request->baseUrl ?>/images/ten_reward.png")
                    }
                })
            } else if (deg > 210 && deg <= 270) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        document.getElementById("award").innerHTML = whichAward(deg);
                        var time = FormatDate();
                        $('.award_li').append("<li>" + time + " 恭喜您抽中60元现金券！</li>");
